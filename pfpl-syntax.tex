...
\footnotetext{I am grateful to Kartik Singal for testing this package.}
...
\end{itemize}
The same option defines \verb|\kw{word}| to typeset \verb|word| as a keyword, in whatever sense is appropriate as determined by the following options.  The package option \verb|[sf]| indicates that operations are to be set in sans serif font, and the package option \verb|[sc]| indicates that they are to be set in small capitals.  These forms are not ordinarily used directly, they are rather for authors to define their own syntax macros, as illustrated for \textsf{PFPL} in the next section.
