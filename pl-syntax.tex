...
\footnotetext{I am grateful to Kartik Singhal for help with testing and improving this package, and to Jan Hoffmann for suggesting to provide a means for suppressing minor arguments from the abstract syntax.}
...
\begin{itemize}
  \item \verb|\Opn{op}<parameter>|: Format an operator as a keyword; use starred form for an operator that is to be set in math mode.  For example, \verb|\Opn{fun}| produces $\Opn{fun}$ and \verb|\Opn*{\lambda}| produces $\Opn*{\lambda}$.  An optional parameter specifies an instance of a family of operators indexed by the given parameter; for example, \verb|\Opn<i>{proj}| produces $\Opn{proj}<i>$.
