...
The command \verb|\Sub{e}{x}{e'}| expands to $\Sub{e}{x}{e'}$, and \verb|\Sub*{e}{x}{e'}| expands to $\Sub*{e}{x}{e'}$, providing pre- and post-fix notation for substitution.  Substitutions may be written as \verb|\empSub| for the empty substitution, $\empSub$, and \verb|\extSub{\gamma}{x}{e}| for the extension $\extSub{\gamma}{x}{e}$ of a substitution $\gamma$ sending $x$ to $e$, and \verb|\extSub*{\gamma}{x}{e}| for $\extSub*{\gamma}{x}{e}$ written in the opposite order.  A substitution, $\gamma$, induces a function, \verb|\appSub{\gamma}|, on abt's performing the simultaneous, capture-avoiding substitution of $\gamma(x)$ for free occurrences of $x$ in its argument, $a$, yielding $\appSub{\gamma}(a)$, or $\appSub*{\gamma}(a)$, in its starred form.
