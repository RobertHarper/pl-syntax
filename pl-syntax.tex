...
\footnotetext{I am grateful to Kartik Singhal for help with testing and improving this package, and to Jan Hoffmann for suggesting an option to suppress minor arguments from the abstract syntax.}
...
\begin{itemize}
  \item \verb|\Opn{op}<parameter>|: Format an operator as a keyword; use starred form for an operator that is to be set in math mode.  For example, \verb|\Opn{fun}| produces $\Opn{fun}$ and \verb|\Opn*{\lambda}| produces $\Opn*{\lambda}$.  An optional parameter specifies the valid indices of a family of operators; for example, \verb|\Opn{proj}<I>| produces $\Opn{proj}<I>$, where $I$ is a finite set of possible indices for projection.
...
The package option \verb|[sf]| indicates that keywords are to be set in sans serif font, and the package option \verb|[sc]| indicates that they are to be set in small capitals.  The package option \verb|[pfpl]| uses the notational conventions from \textsf{PFPL} in which symbol parameters to operators are typeset in square brackets, and their minor arguments are typeset in curly braces.  The package option \verb|[nominor]| suppresses minor arguments to abt's in abstract syntax; the concrete forms are not affected by this option.  Finally, the package option \verb|[nosyn]| omits the collection of syntax macros described below.
...
The commands for typesetting the many language constructs in \textsf{PFPL}, plus some others, are organized into a collection of tables grouped according to type structure.  The tables display the abstract and concrete syntax for a language construct, and give the \LaTeX{} command used to create them.  The minor arguments are generally required for the abstract syntax (but are suppressed when the package option \verb|[nominor]| is in effect), but may be omitted in the concrete syntax (starred form), which is more free-wheeling.  For the variadic forms in Figures~\ref{fig:prod} and~\ref{fig:sum}, the notation $\tau_I$, for $I$ an index set, stands for the finite map $(\finMap{I}{i}{\tau})$; the notation $e_I$ is defined analogously.
  ...
  \begin{small}
    \begin{displaymath}
      \begin{array}{l@{\qquad}l@{\qquad}l@{\qquad}l}
        ...
        \projEx<i>[\tau_1][\tau_2]{e}     & \projEx*<i>{e}           & \verb|\projEx<i>[\tau_1][\tau_2]{e}| \quad (i=1,2)            \\
        ...
        \multicolumn{3}{r}{\UseVerb{splitExVrb}} \\[1ex]
  ...
  \begin{small}
    \begin{displaymath}
      \begin{array}{l@{\qquad}l@{\qquad}l@{\qquad}l}
        ...
        \lamEx[\tau_1][\tau_2]{x}{e}        & \lamEx*{x}{e}               & \verb|\lamEx[\tau_1][\tau_2]{x}{e}|           \\
        \appEx[\tau_1][\tau_2]{e_1}{e_2}    & \appEx*{e_1}{e_2}           & \verb|\appEx[\tau_1][\tau_2]{e_1}{e_2}| \\[1ex]
  ...
  \begin{small}
    \begin{displaymath}
      \begin{array}{l@{\qquad}l@{\qquad}l@{\qquad}l}
        ...
        \papEx[\tau_1][\tau_2]{e_1}{e_2} & \papEx*{e_1}{e_2}        & \verb|\papEx[\tau_1][\tau_2]{e_1}{e_2}| \\[1ex]
...
\begin{figure}
  ...
  \begin{displaymath}
    \begin{array}{l@{\qquad}l@{\qquad}l@{\qquad}l}
      ...
      \lamCn[\kappa_1][u][\kappa_2]{u}{c} & \lamCn*{u}{c} & \verb|\lamCn[\kappa_1][u][\kappa_2]{u}{c}| \\
      \appCn[\kappa_1][u][\kappa_2]{c}{c_1} & \appCn*[x][x][x]{c}{c_1} & \verb|\appCn[\kappa_1][u][\kappa_2]{c}{c_1}| \\
      \prodKd{\kappa_1}[u]{\kappa_2} & \prodKd*{\kappa_1}[u]{\kappa_2} & \verb|\prodKd{\kappa_1}[u]{\kappa_2}| \\
      \pairCn[\kappa_1][u][\kappa_2]{c_1}{c_2} & \pairCn*{c_1}{c_2} & \verb|\pairCn[\kappa_1][u][\kappa_2]{c_1}{c_2}| \\
      \projCn<i>[\kappa_1][u][\kappa_2]{c} & \projCn*<i>{c} & \verb|\projCn<i>[\kappa_1][u][\kappa_2]{c}|
      \end{array}
  \end{displaymath}

  \caption{Kinds and Constructors}
  \label{fig:con-knd}
\end{figure}
  ...
  \begin{small}
    \begin{displaymath}
      \begin{array}{l@{\qquad}l@{\qquad}l@{\qquad}l}
        ...
        \suspEx[\tau]{e}        & \suspEx*{e}        & \verb|\suspEx[\tau]{e}|        \\
        \forceEx[\tau]{e}       & \forceEx*{e}       & \verb|\forceEx[\tau]{e}|   \\[1ex]
...
\begin{figure}
  ...
  % shouldn't there be optional type arguments?
  \begin{small}
    \begin{displaymath}
      \begin{array}{l@{\qquad}l@{\qquad}l@{\qquad}l}
        \newclsCmd[\tau]              & \newclsCmd*[\tau]     & \verb|\newclsCmd[\tau]| \\
  ...
  \begin{small}
    \begin{displaymath}
      \begin{array}{l@{\qquad}l@{\qquad}l@{\qquad}l}
        ...
        \isMd[S][M]{N}                 & \isMd*{N}            & \verb|\isMd[S][M]{N}|                          \\
        \asMd[S][M]{N}                & \asMd*{N}           & \verb|\asMd[S][M]{N}|                         \\[1ex]
        ...
        \projMd<i>[S_1][x][S_2]{M}     & \projMd*<i>{M}       & \verb|\projMd<i>[S_1][x][S_2]{M}| \quad (i=1,2) \\[1ex]
        \funSg{S_1}[x]{S_2}             & \funSg*{S_1}[x]{S_2}  & \verb|\funSg{S_1}[x]{S_2}|                       \\
        \funSg{S_1}{S_2}                & \funSg*{S_1}{S_2}     & \verb|\funSg{S_1}{S_2}| \\
        \funMd{S_1}{x}[S_2]{M_2}       & \funMd*{S_1}{x}[S_2]{M_2} & \verb|\funMd{S_1}{x}[S_2]{M_2}|            \\
        \instMd[S_1][x][S_2]{M_1}{M_2} & \instMd*{M_1}{M_2}   & \verb|\instMd[S_1][x][S_2]{M_1}{M_2}|
      \end{array}
    \end{displaymath}
  \end{small}
